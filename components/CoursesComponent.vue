<template>
  <section class="course py-[72px] xl:pt-[40px] bg-gray" id="plan">
    <div class="content">
      ...
      <div class="course__grid grid grid-cols-3 xl:grid-cols-2 md:grid-cols-1 gap-6 justify-around">
        <div
            class="course__item bg-white px-6 py-8 rounded-[20px] flex flex-col justify-between relative w-full max-w-[450px] mx-auto"
            v-for="item in courses" :key="item.id">

          <div v-if="item.popular"
               class="course__popular bg-black flex items-center justify-center gap-2 absolute top-0 left-0 py-3 rounded-t-[20px] w-full">
            ...
            <span class="text-[16px] text-white font-[600]">Самый популярный</span>
          </div>

          <h3 class="course__title font-[600] text-2xl mb-5 mt-8">
            {{ item.title }}
          </h3>
          <div class="course__advantages">
            <div v-for="el in item.advantages" class="flex gap-2 mb-3">
              <svg width="24px" height="24px" viewBox="0 0 48 48" class="min-w-[24px]">
                <g id="Shopicon">
                  <polygon
                      points="31.048,12 31.048,8 8,8 8,40 40,40 40,20.655 36,20.655 36,36 12,36 12,12 	"/>
                  <polygon
          ...
          <div class="course__price grow basis-0 flex items-end mt-10">
            <div class="flex items-start">
              <span class="new text-3xl font-[600]">от {{ item.priceNew }} <span
                  class="new text-sm line-through mt-0.5 mr-2">₽/мес.</span></span>
          ...
          <div class="course__get mt-6">
            <a href="javascript:jivo_api.open()"
               class="flex justify-center bg-purple text-white hover:bg-purple-hover text-sm border-0 rounded-xl px-5 py-3 w-full">
              Записаться
            </a>
          </div>
        </div>
...
const courses = [
  {
    ...
    advantages: [
      'Одна личная встреча',
      'Обзор Вашего аккаунта',
      'Получение руководства по внутренней рекламе',
    ],
    priceNew: 5000,
    priceOld: 10000,
    popular: false,
